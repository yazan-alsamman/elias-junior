...
const { pingAtsFastapi } = require('./services/atsAnalyzeUpload');

app.get('/health', async (req, res) => {
  const atsPing = await pingAtsFastapi();
  res.json({
    ok: true,
    service: 'careerpath-api',
    version: 2,
    saveAnalysisRoute: true,
    atsFastapiUrl: process.env.ATS_FASTAPI_URL || null,
    atsReachable: atsPing.ok,
    atsPingReason: atsPing.ok ? null : atsPing.reason,
  });
});
