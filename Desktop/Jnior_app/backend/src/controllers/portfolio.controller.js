...
async function upsertPortfolio(req, res) {
  try {
    ...
    const userSnapshot = sanitizePreviewSnapshot(req.body.previewSnapshot);
    ...
    const baseFromRequest =
      requestHost && !/^localhost|127\.0\.0\.1/i.test(String(requestHost).split(':')[0])
        ? `${requestProto}://${requestHost}`.replace(/\/$/, '')
        : null;
    const baseRaw =
      process.env.PUBLIC_BASE_URL || baseFromRequest || `http://localhost:${port}`;
    ...
    const fromCv =
      parsedCv &&
      portfolioSnapshotFromParsedCv(parsedCv, {
        githubUsername: gh,
        templateId: String(templateId || 'classic'),
        projectRepos,
      });
    const previewSnapshot = sanitizePreviewSnapshot(
      fromCv ? mergePortfolioSnapshots(fromCv, userSnapshot || {}) : userSnapshot,
    );
